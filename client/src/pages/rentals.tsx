...
import { Plus, Search, Eye, Edit, X, Package, FileText, Clock, DollarSign, AlertTriangle, Calendar, MessageSquare, Download, Filter, Repeat, Star, ArrowRightLeft, History } from "lucide-react";
import { SubstitutionHistory } from "@/components/substitution-history";
import { Link } from "wouter";
import { useToast } from "@/hooks/use-toast";
import { apiRequest, queryClient } from "@/lib/queryClient";
import type { RentalWithDetails, Supplier } from "../types";
...
export default function Rentals() {
  ...
  const [substitutionHistoryDialog, setSubstitutionHistoryDialog] = useState({
    open: false,
    rental: null as RentalWithDetails | null,
  });
  ...
  // Mutation to create a new equipment substitution
  const createSubstitutionMutation = useMutation({
    mutationFn: async (data: {
      rentalId: string;
      oldEquipmentName: string;
      ...
      const response = await apiRequest("POST", `/api/rentals/${data.rentalId}/substitutions`, data);
      return response.json();
    },
    onSuccess: () => {
      toast({
        title: "Sucesso",
        description: "Substituição de equipamento registrada!",
      });
      queryClient.invalidateQueries({ queryKey: ["/api/rentals"] });
    ...
    onError: () => {
      toast({
        title: "Erro",
        description: "Falha ao registrar substituição de equipamento",
        variant: "destructive",
      });
    },
  });
  ...
  return (
    ...
    >
                          ...
                          <Button
                            ...
                            onClick={() => {
                              setSubstitutionDialog({
                                ...
                                open: true,
                                rental,
                                oldEquipmentName: rental.equipmentName,
                              });
                            }}
                            data-testid={`button-substitution-${rental.id}`}
                          >
                            <ArrowRightLeft className="w-4 h-4 text-blue-600" />
                          </Button>
                        )}

                        {/* Substitution History Button */}
                        <Button
                          variant="ghost"
                          size="sm"
                          onClick={() => {
                            setSubstitutionHistoryDialog({
                              open: true,
                              rental,
                            });
                          }}
                          data-testid={`button-history-${rental.id}`}
                        >
      ...
      {/* Equipment Substitution Dialog */}
      <Dialog open={substitutionDialog.open} onOpenChange={(open) => setSubstitutionDialog({ ...substitutionDialog, open })}>
        <DialogContent className="max-w-2xl max-h-[90vh] overflow-y-auto">
          <DialogHeader>
            <DialogTitle>Registrar Substituição de Equipamento</DialogTitle>
            <p className="text-sm text-muted-foreground">
              Documente a troca de equipamento feita pela locadora
            </p>
          </DialogHeader>
          <div className="space-y-4">
            <div className="grid grid-cols-2 gap-4">
              <div>
                <Label>Equipamento Original</Label>
                <Input
                  value={substitutionDialog.oldEquipmentName}
                  onChange={(e) => setSubstitutionDialog({ ...substitutionDialog, oldEquipmentName: e.target.value })}
                  placeholder="Nome do equipamento substituído"
                />
              </div>
              <div>
                <Label>Novo Equipamento</Label>
                <Input
                  value={substitutionDialog.newEquipmentName}
                  onChange={(e) => setSubstitutionDialog({ ...substitutionDialog, newEquipmentName: e.target.value })}
            ...
            <div className="grid grid-cols-2 gap-4">
              <div>
                ...
                <Select
                  value={substitutionDialog.substitutionReason}
                  onValueChange={(value) => setSubstitutionDialog({ ...substitutionDialog, substitutionReason: value })}
                >
                  <SelectTrigger>
              ...
              <div>
                ...
                <Input
                  type="datetime-local"
                  value={substitutionDialog.substitutionDate}
                  onChange={(e) => setSubstitutionDialog({ ...substitutionDialog, substitutionDate: e.target.value })}
                />
              </div>
            </div>

            <div>
              <Label>Responsável da Locadora</Label>
              <Input
                value={substitutionDialog.supplierResponsible}
                onChange={(e) => setSubstitutionDialog({ ...substitutionDialog, supplierResponsible: e.target.value })}
                placeholder="Nome do funcionário que fez a substituição"
              />
            </div>
            ...
            <div>
              ...
              <textarea
                ...
                value={substitutionDialog.notes}
                onChange={(e) => setSubstitutionDialog({ ...substitutionDialog, notes: e.target.value })}
                placeholder="Suas observações sobre a substituição..."
              />
            </div>

            <div>
              <Label>Observações da Locadora</Label>
              <textarea
                className="w-full min-h-[80px] p-2 border rounded"
                value={substitutionDialog.supplierNotes}
                onChange={(e) => setSubstitutionDialog({ ...substitutionDialog, supplierNotes: e.target.value })}
                placeholder="Observações fornecidas pela locadora..."
              />
            </div>

            <div className="flex items-center space-x-2">
              <Checkbox
                id="responsibility-shift"
                checked={substitutionDialog.responsibilityShift}
                onCheckedChange={(checked) => setSubstitutionDialog({ ...substitutionDialog, responsibilityShift: checked as boolean })}
              />
              <Label htmlFor="responsibility-shift" className="text-sm">
                Houve transferência de responsabilidade/custos para mim
              </Label>
            </div>

            {substitutionDialog.responsibilityShift && (
              <div>
                <Label>Custos Adicionais (R$)</Label>
                <Input
                  type="number"
                  step="0.01"
                  value={substitutionDialog.additionalCosts}
                  onChange={(e) => setSubstitutionDialog({ ...substitutionDialog, additionalCosts: e.target.value })}
                  placeholder="0.00"
                />
              </div>
            )}
            ...
            <div className="space-y-2">
              <Label>Fotos da Entrega (URLs)</Label>
              <Input
                ...
                onKeyPress={(e) => {
                  if (e.key === 'Enter') {
                    ...
                    if (url) {
                      setSubstitutionDialog({
                        ...substitutionDialog,
                        deliveryPhotos: [...substitutionDialog.deliveryPhotos, url]
                      });
              ...
              <div className="flex flex-wrap gap-2">
                {substitutionDialog.deliveryPhotos.map((photo, index) => (
                  <div key={index} className="bg-gray-100 p-2 rounded text-xs max-w-[200px] truncate">
                    {photo}
                    <Button
                      variant="ghost"
                      size="sm"
                      ...
                      onClick={() => {
                        setSubstitutionDialog({
                          ...substitutionDialog,
                          deliveryPhotos: substitutionDialog.deliveryPhotos.filter((_, i) => i !== index)
                        });
                      }}
                    >
                      ×
            ...
            <div className="space-y-2">
              <Label>Fotos do Recebimento (URLs)</Label>
              <Input
                placeholder="URL da foto (ex: https://...)"
                onKeyPress={(e) => {
                  if (e.key === 'Enter') {
                    ...
                    if (url) {
                      setSubstitutionDialog({
                        ...substitutionDialog,
                        receiptPhotos: [...substitutionDialog.receiptPhotos, url]
                      });
                      e.currentTarget.value = '';
                    }
                  }
              ...
              />
              <div className="flex flex-wrap gap-2">
                {substitutionDialog.receiptPhotos.map((photo, index) => (
                  <div key={index} className="bg-gray-100 p-2 rounded text-xs max-w-[200px] truncate">
                    {photo}
                    <Button
                      variant="ghost"
                      ...
                      onClick={() => {
                        setSubstitutionDialog({
                          ...substitutionDialog,
                          receiptPhotos: substitutionDialog.receiptPhotos.filter((_, i) => i !== index)
                        });
                      }}
                    >
                      ×
          ...
          <div className="flex justify-end space-x-2 pt-4">
            <Button variant="outline" onClick={() => setSubstitutionDialog({ ...substitutionDialog, open: false })}>
              Cancelar
            </Button>
            <Button
              onClick={() => {
                createSubstitutionMutation.mutate({
                  rentalId: substitutionDialog.rental!.id,
                  oldEquipmentName: substitutionDialog.oldEquipmentName,
                  newEquipmentName: substitutionDialog.newEquipmentName,
                  substitutionReason: substitutionDialog.substitutionReason,
                  supplierResponsible: substitutionDialog.supplierResponsible,
                  substitutionDate: new Date(substitutionDialog.substitutionDate),
                  deliveryPhotos: substitutionDialog.deliveryPhotos,
                  receiptPhotos: substitutionDialog.receiptPhotos,
                  notes: substitutionDialog.notes,
                  supplierNotes: substitutionDialog.supplierNotes,
                  responsibilityShift: substitutionDialog.responsibilityShift,
                  additionalCosts: substitutionDialog.additionalCosts,
                });
              }}
              disabled={!substitutionDialog.newEquipmentName || !substitutionDialog.substitutionReason || !substitutionDialog.substitutionDate}
            >
      ...
      {/* Substitution History Dialog */}
      <Dialog open={substitutionHistoryDialog.open} onOpenChange={(open) => setSubstitutionHistoryDialog({ ...substitutionHistoryDialog, open })}>
        <DialogContent className="max-w-6xl max-h-[90vh] overflow-hidden">
          <DialogHeader>
            <DialogTitle>Histórico de Substituições</DialogTitle>
            <p className="text-sm text-muted-foreground">
              {substitutionHistoryDialog.rental && (
                <>Equipamento: {substitutionHistoryDialog.rental.equipmentName}</>
              )}
            </p>
          </DialogHeader>
          <div className="overflow-y-auto max-h-[75vh] pr-2">
            {substitutionHistoryDialog.rental && (
              <SubstitutionHistory
                rentalId={substitutionHistoryDialog.rental.id}
                equipmentName={substitutionHistoryDialog.rental.equipmentName}
              />
            )}
          </div>
        </DialogContent>
